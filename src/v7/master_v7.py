...
import sys, os, argparse, glob, json, time, logging
...
logger = logging.getLogger("v7.master")
...
def run_dry_run(args):
    ...
    for cp in engine.all_checkpoints:
        logger.info(f"    [{cp.id}] {cp.name} (type={cp.check_type.value}, route={cp.route.value}→resolved={engine.resolve_route(cp).value})")
    ...
    if dxf_files:
        for f in dxf_files[:3]:
            ...
            logger.info(f"  {info.readable_name}: discipline={info.discipline}, {len(info.text_entities)} text entities")
            if info.text_content:
                std_result = checker.check(info.text_content, info.discipline)
                logger.info(f"    标准化校验: {std_result.passed_checks}/{std_result.total_checks} 通过")
    else:
        logger.info("  未找到DXF文件，使用模拟数据")
        logger.info("  分类规则测试: ", end="")
...
def main():
    ...
    # 默认 0 表示从 .env 读取 V7_REVIEW_PORT
    try:
        from v7.config_loader import get_config as _get_cfg
        _default_review_port = _get_cfg().review_port
    except (ImportError, ValueError):
        _default_review_port = int(os.environ.get("V7_REVIEW_PORT", "8080"))
    parser.add_argument("--port", type=int, default=_default_review_port, help="Web端口")
