...
class ExcelReporter:
    ...
    def __init__(self, conflicts_json_path: str):
        try:
            ...
            raise ValueError(f"冲突数据JSON格式错误: {e}") from e
        except FileNotFoundError:
            raise ValueError(f"冲突数据文件不存在: {conflicts_json_path}")
    ...
    def generate(self, output_path: str = "coordination_sheet.xlsx"):
        ...
        try:
            ...
            raise PermissionError(f"无法写入文件（可能被占用）: {output_path}")
        except IOError as e:
            raise IOError(f"保存Excel失败: {e}") from e
