        ...
        for i in range(len(spatial_metas)):
            for j in range(i + 1, len(spatial_metas)):
                ...
                if a_vals and b_vals:
                    ...
                    if math.isclose(min_diff, 0, abs_tol=0.5) and math.isclose(max_diff, 0, abs_tol=0.5):
                        continue
                    # 系统性偏移（各标高差值一致）→ 不同基准面
                    if len(a_vals) > 1 and len(b_vals) > 1 and math.isclose(min_diff, max_diff, abs_tol=0.5):
                        issues.append(CrossDrawingIssue(
                        ...
                        continue
                # 非系统性差异：检查单值不匹配
                if a_vals and b_vals and math.isclose(min(a_vals), min(b_vals), abs_tol=0.5):
                    continue
