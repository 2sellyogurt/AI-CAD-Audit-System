...
def review_architecture():
    ...
    issues = [
        ...
        },
        {
            ...
            "finding": "项目位于夏热冬冷气候区，公建甲类。外窗采用6Low-E+12氩气+6透明中空玻璃(29mm)，断热铝合金单框，气密性不低于7级。外窗传热系数K值未在图纸中明确标注，仅标注了玻璃构造，缺少整窗K值计算书。",
...
def review_curtain_wall():
    ...
    issues = [
        ...
        },
        {
            ...
            "finding": "按50年基本风压校核，幕墙抗风压设计值需按GB50009计算。幕墙图纸中未标注设计风压值，风压高度变化系数和阵风系数的选取未知。",
            "fix": "补充幕墙抗风压设计值计算书，明确基本风压0.60kN/m²、高度变化系数、阵风系数及体型系数的取值，验算49.7m高度处风荷载标准值。"
        },
        {
            "id": "CW-006", "severity": "B",
            "standard": "GB50011-2010(2016版) 建筑抗震设计规范",
            "finding": "抗震设防烈度6度（0.05g），幕墙作为非结构构件需按GB50011第13章进行抗震设计。幕墙与主体结构的连接节点应能承受多遇地震下的惯性力。图纸中未见幕墙抗震构造措施的专项说明（如平面内变形能力要求、层间位移角适配）。",
...
def review_landscape():
    ...
    issues = [
        ...
        },
        {
            ...
            "fix": "补充年径流总量控制率的计算，确认海绵城市设计满足当地规划条件（年径流总量控制率一般要求≥70%）。"
        },
    ]
    return issues
...
def review_doors_windows():
    ...
    issues = [
        ...
        },
        {
            ...
            "standard": "门窗设计计算书（设计说明引用）",
            "finding": "图纸目录中包含门窗计算书但内容未被提取。门窗计算书应包含：外窗抗风压计算、玻璃挠度验算、型材强度验算、开启扇五金件受力分析等。",
...
def review_outdoor_utilities():
    ...
    issues = [
        ...
        },
        {
            ...
            "finding": "校园10kV电力管线和弱电线缆的走向仅通过文件命名间接体现，管线与排水管的水平间距未经系统校核。室外管综图中仅见给排水专业的井编号和埋深数据，电力/弱电/燃气管线未参与管综协同。",
    ...
    issues = [
        {
            ...
            "fix": "补充光伏组件布置详图，明确组件倾角、安装方式、支架系统的固定方式及与防水屋面的连接节点。"
        },
        {
...
def gen_scene3(doc, spatial, findings):
    ...
    doc.add_paragraph(
        "本报告由AI LLM代理对项目全部施工图进行强制性条文符合性审查。"
