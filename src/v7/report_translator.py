...
OUT_MD = os.path.join(BASE, "output_v7.0", f"AI-{datetime.now().strftime('%Y%m%d')}-{os.environ.get('V7_PROJECT_CODE', '001')}_审查报告.md")
OUT_TXT = os.path.join(BASE, "output_v7.0", f"AI-{datetime.now().strftime('%Y%m%d')}-{os.environ.get('V7_PROJECT_CODE', '001')}_审查报告.txt")
...
def generate_report():
    ...
    report_no = f"AI-{datetime.now().strftime('%Y%m%d')}-001"
    ...
    # 封面
    # ============================================================
    w(f"# {os.environ.get('V7_PROJECT_NAME', '施工图审查项目')}")
    w(f"# 施工图AI智能审查报告")
    w(f"")
    w(f"> 报告编号: **{report_no}**")
...
def generate_final_report(checklist_path, spatial_path, uai_findings, severity_adjust, output_dir):
    ...
    RN = f"AI-{datetime.now().strftime('%Y%m%d')}-{os.environ.get('V7_PROJECT_CODE', '001')}"
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    # 封面
    w.append(f"# {os.environ.get('V7_PROJECT_NAME', '施工图审查项目')}")
