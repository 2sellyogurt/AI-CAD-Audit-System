...
def index():
    ...
    return render_template_string(
        ...
        PROJECT_NAME=app.config.get("PROJECT_NAME", "施工图审查项目"),
        issues=all_issues,
        stats=stats,
        sev_filter=severity,
...
def run_server(problem_pool, project_name: str = "", port: int = 0,
               audit_log_dir: str = "", debug: bool = False):
    # 端口配置 - 优先 .env 中的 V7_REVIEW_PORT
    if port <= 0:
        try:
            from v7.config_loader import get_config
            port = get_config().review_port
        except (ImportError, ValueError):
            port = int(os.environ.get("V7_REVIEW_PORT", "8080"))
...
if __name__ == "__main__":
    ...
    try:
        from v7.config_loader import get_config
        _default_port = get_config().review_port
    except (ImportError, ValueError):
        _default_port = int(os.environ.get("V7_REVIEW_PORT", "8080"))
