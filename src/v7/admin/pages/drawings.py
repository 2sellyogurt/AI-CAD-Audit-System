...
.drop-zone { border:2px dashed #ccc; border-radius:8px; padding:36px 20px; text-align:center;
             color:#555; cursor:pointer; transition:all .2s; margin-bottom:12px; }
...
from v7.constants import DISCIPLINE_LABELS, DRAWING_STATUS_LABELS as STATUS_LABELS
...
<div class="stats-row">
  ...
  <div class="stat-card" style="border-left-color:#3498db; cursor:pointer;" onclick="loadProjects()" title="点击刷新统计">
    <div class="stat-num" style="font-size:20px;">🔄</div>
    <div class="stat-label">刷新统计</div>
  </div>
</div>
...
def _handle_upload_drawing(body):
    ...
    if review_dir and os.path.isdir(review_dir):
        ...
        dest_dir = os.path.join(root, "output_v7.0", "dxf", safe_name)
    ...
    for r in rows:
        ...
        if discipline == "unknown" and r["file_path"]:
            try:
                ...
                import logging
                logging.getLogger(__name__).debug(f"文本内容分类失败: {r['filename']}, {e}")
...
def _handle_delete_drawing(did):
    ...
    if row["file_path"] and os.path.isfile(row["file_path"]):
        try:
            ...
            logger.warning(f"删除图纸物理文件失败 {row['file_path']}: {e}")
