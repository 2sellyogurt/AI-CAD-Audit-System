    ...
    for fn in os.listdir(CACHE_DIR):
        ...
        if os.path.isfile(fp):
            try:
                ...
                logger.warning(f"删除缓存文件失败 {fp}: {e}")
    ...
    for fn in os.listdir(CACHE_DIR):
        ...
        if os.path.isfile(fp):
            try:
                ...
                logger.warning(f"清理过期缓存失败 {fp}: {e}")
...
def _api_restore_db(request_body=None):
    ...
    # 路径穿越防护：确保backup_path在备份目录内
    backup_dir = os.path.join(V7_ROOT, "backups")
    backup_dir = os.path.realpath(backup_dir)
    real_path = os.path.realpath(backup_path)
    if not real_path.startswith(backup_dir + os.sep) and real_path != backup_dir:
        return _json(403, {"ok": False, "error": "非法路径：只能在备份目录内恢复"})
    if not real_path.endswith(".db"):
        return _json(400, {"ok": False, "error": "只能恢复 .db 文件"})
    try:
...
.sys-stat-item .s-label{font-size:11px;color:#555;margin-bottom:2px;text-transform:uppercase;letter-spacing:.5px}
...
.sys-table th{background:#f0f2f5;color:#1a1a2e;font-weight:600;padding:8px 10px;text-align:left;border-bottom:2px solid #ddd;white-space:nowrap}
.sys-table td{padding:7px 10px;border-bottom:1px solid #eee;vertical-align:middle;color:#333}
    ...
    <div class="sys-card-body">
      ...
      <div style="max-height:360px;overflow-y:auto">
        <table class="sys-table">
          ...
          <tbody id="sys-cache-tbody"><tr><td colspan="5" style="text-align:center;color:#555;padding:20px">加载中...</td></tr></tbody>
        </table>
      </div>
    </div>
  </div>
  ...
  <div class="sys-card">
    ...
    <div class="sys-card-body">
      <div class="sys-toolbar">
        ...
        <span style="font-size:11px;color:#555" id="sys-log-info">—</span>
      </div>
  ...
  <div class="sys-card sys-card-full">
    ...
    <div class="sys-card-body">
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:#f8f9fc;border-radius:8px">
        ...
        <span style="font-size:12px;color:#555">DB Schema v<span id="sys-schema-ver-display">—</span></span>
      </div>
      ...
      <div style="margin-top:20px;border-top:1px solid #eee;padding-top:16px">
        ...
        <div style="max-height:360px;overflow-y:auto">
          <table class="sys-table">
            ...
            <tbody id="sys-version-tbody"><tr><td colspan="4" style="text-align:center;color:#555;padding:20px">加载中...</td></tr></tbody>
          </table>
        </div>
      </div>
    </div>
...
function sysRefreshCaches() {
  sysApi('GET', '/caches').then(function(d) {
    ...
    if (!d.caches || d.caches.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#555;padding:20px">暂无缓存文件</td></tr>';
      return;
    }
    var html = '';
...
function sysRefreshVersions() {
  sysApi('GET', '/config-versions').then(function(d) {
    ...
    if (!d.versions || d.versions.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;color:#555;padding:20px">暂无版本记录</td></tr>';
      return;
    }
