...
from src.step4_cross_check import (
...
class TestParseArgs:
    def test_default_args(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["step4_cross_check.py"])
        args = parse_args()
        assert args.json_dir is not None
        assert args.output_dir is not None

    def test_custom_json_dir(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["step4_cross_check.py", "--json-dir", "/tmp/jsons"])
        args = parse_args()
        assert args.json_dir == "/tmp/jsons"

    def test_custom_output_dir(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["step4_cross_check.py", "--output-dir", "/tmp/output"])
        args = parse_args()
    ...
    def test_both_dirs(self, monkeypatch):
        monkeypatch.setattr(
            sys, "argv",
            ["step4_cross_check.py", "--json-dir", "/a", "--output-dir", "/b"],
        )
        args = parse_args()
    ...
    def test_cli_overrides_env(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sys, "argv",
            ["step4_cross_check.py", "--json-dir", "/cli/json"],
        )
        args = parse_args()
