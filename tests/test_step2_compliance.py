...
from src.step2_compliance import (
...
class TestReplaceUuidInTextExtra:
    ...
    def test_single_char_after_underscore(self):
        text = "aaa_bbb-ccc-ddd-eee-fff_x_unified.json"
        loader = MockLoader()
...
class TestCleanFilenamesInResultsExtra:
    ...
    def test_location_field(self):
        data = {"location": "def_结构梁图_unified.json", "detail": "问题"}
        result = clean_filenames_in_results(data, self.loader)
        assert result["location"] == "结构梁图"
...
class TestValidateEvidenceChainExtra:
    ...
    def test_mixed_issue_and_non_issue(self):
        data = {
            "name": "梁截面",
            "description": "净高不足",
            "file": "建筑平面图",
        ...
        result = _validate_evidence_chain(data)
        assert result["confidence_level"] == "L3-完整"
        assert "confidence_level" not in {"name": "梁截面"} or True

    def test_l0_no_evidence_with_real_file(self):
        data = {
            "description": "管线问题",
            "file": "建筑平面图",
        }
...
class TestParseArgs:
    def test_default_args(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["step2_compliance.py"])
        from src.step2_compliance import parse_args
        args = parse_args()
        assert args.json_dir is not None
        assert args.output_dir is not None

    def test_custom_json_dir(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["step2_compliance.py", "--json-dir", "/tmp/jsons"])
        from src.step2_compliance import parse_args
        args = parse_args()
        assert args.json_dir == "/tmp/jsons"

    def test_custom_output_dir(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["step2_compliance.py", "--output-dir", "/tmp/output"])
        from src.step2_compliance import parse_args
        args = parse_args()
    ...
    def test_both_dirs(self, monkeypatch):
        monkeypatch.setattr(
            sys, "argv",
            ["step2_compliance.py", "--json-dir", "/a", "--output-dir", "/b"],
        )
        from src.step2_compliance import parse_args
        args = parse_args()
    ...
    def test_cli_overrides_env(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sys, "argv",
            ["step2_compliance.py", "--json-dir", "/cli/json"],
        )
        from src.step2_compliance import parse_args
        args = parse_args()
